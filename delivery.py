...
class DeliveryRobot():
    ...
    def __init__(self) -> None:
        ...
        self.mypub = rospy.Publisher('led_control', Byte, queue_size = 1)
    ...
    def set_state(self, state: str):
        self.state = state
        print("-----------------------------"+state+"--------------------------------")
        if state in ["robot_start", "have_product", "home_cap_close_wait", "product_wait"]:
            self.mypub.publish(1)
        elif state in ["move_to_delivery_point", "move_to_home_point"]:
            self.mypub.publish(2)
        elif state in ["on_delivery_point", "client_pickup_wait", "client_cap_close_wait"]:
            self.mypub.publish(3)
